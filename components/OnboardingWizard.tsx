...
import {
  ...
  AiPresetResultCard,
  OnboardingAlexLoadingCard,
  type SaveUiState,
} from '@/components/ai/AiPresetCards';
...
const SPLIT_LABELS_MSG: Record<SplitKey, string> = {
  full_body: 'full body',
    ...
    <>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        {phase >= 4 && (
          ...
          >
            Push Day · 5 exercises · Generated by Alex
          </div>
        )}
      </div>
    </>
...
function buildPresetPrompt(d: Draft): string {
  const split = d.splitPreference ? SPLIT_LABELS_MSG[d.splitPreference] : 'general';
  const exp = d.experienceLevel ?? 'beginner';
  const goals =
    d.goal.length > 0 ? d.goal.map((g) => GOAL_LABELS[g]).join(', ') : 'general fitness';
  return `Create a ${split} workout preset for a ${exp} level athlete whose goals are ${goals}. They train ${d.trainingDaysPerWeek} days per week. Make it practical and effective.`;
}
...
function FirstPresetStep({
  ...
  const [preset, setPreset] = useState<AiGeneratedPreset | null>(null);
  ...
  async function generate() {
    ...
    setPreset(null);
    try {
      const msg = buildPresetPrompt(draft);
      const res = await postAiChatApi(msg);
      if (!res.preset) {
        setErr('Could not generate a preset. Try again.');
        return;
      }
  ...
  async function savePreset() {
    if (!preset) return;
    const exerciseIds = preset.exercises.map((e) => e.exerciseId).filter((id): id is string => typeof id === 'string' && id.length > 0);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {loading && <OnboardingAlexLoadingCard />}
      ...
      {preview && preset ? <AiPresetPreviewSheet preset={preset} onClose={() => setPreview(false)} /> : null}
    </div>
  );
}
        ...
        <StepAnim stepKey={animKey} dir={dir}>
          ...
          {step === 12 && (
            <FirstPresetStep
              ...
              onAfterSave={() => goTo(FINISH_STEP, 'fwd')}
            />
          )}
